...
class SmartCache {
  ...
  async isExpired(request, response) {
    ...
    const isCritical = CACHE_CONFIG.critical.some(pattern => pathname.includes(pattern));
    const isDynamic = CACHE_CONFIG.dynamic.some(pattern => pathname.includes(pattern));

    if (isCritical) {
      const maxAge = CACHE_CONFIG.maxAge.critical;
      const cachedTime = new Date(response.headers.get('sw-cache-time') || 0).getTime();
  ...
  async cleanup() {
    ...
    if (criticalKeys.length > CACHE_CONFIG.maxEntries.critical) {
      const toDelete = criticalKeys.slice(0, criticalKeys.length - CACHE_CONFIG.maxEntries.critical);
    ...
    if (dynamicKeys.length > CACHE_CONFIG.maxEntries.dynamic) {
      const toDelete = dynamicKeys.slice(0, dynamicKeys.length - CACHE_CONFIG.maxEntries.dynamic);
      await Promise.all(toDelete.map(request => this.cache.delete(request)));
...
console.log('  📌 CRITICAL (cache-first):', CACHE_CONFIG.critical.length, 'assets');
console.log('  🔄 DYNAMIC (network-first):', CACHE_CONFIG.dynamic.length, 'assets');
console.log('  🚫 VOLATILE (network-only):', CACHE_CONFIG.volatile.length, 'endpoints');
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    smartCache.init().then(() => {
      ...
      const criticalRequests = CACHE_CONFIG.critical.map(url =>
